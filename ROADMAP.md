...
**Status**: Core capability roadmap (Phases 1-7), architectural refactoring, and initial JNI performance optimizations landed; P0 correctness contracts (handle lifetime leases, lossless streaming, prompt admission) actively being closed.
...
**Completed**: Cosine similarity index in direct SQLite, text chunking, and llama.cpp embedding extraction.
...
### 5. Structured Data Connectors (Read-Only) 🟡 DISABLED IN REGISTRY
...
**Completed**: Android Content Provider bridges for contacts, calendar, and SMS threads exist, but are disabled (`isAvailable = false`) in AgentTools registry and permissions are absent from manifest.
...
### 7. On-Device Personalization (LoRA) 🟡 RUNTIME SUBSTRATE IMPLEMENTED
Model adapts to user's writing style and vocabulary via LoRA adapters.
**Dependencies**: llama.cpp LoRA support maturity
**Status**: Native adapter loading and clearing substrate implemented; personalization UX and training flow remain planned.
...
|----|----------|---------------|--------|
| D1 | `web_search` risk: SAFE vs CONFIRM? | CONFIRM (requires explicit user confirmation) | **Resolved** — set to CONFIRM by security remediation |
