...
class ModelManager(
    ...
    suspend fun switchModel(modelId: String): Boolean {
        ...
        return when (val resolved = modelStorageManager.resolveActiveModel(modelId)) {
    ...
    private fun nativeLoadRejectionMessage(
        ...
        return when {
            model.bytes > MODEL_LOAD_HARD_CAP_BYTES ->
                "Model ${model.id} is $modelSize, above this build's ${FormatUtils.formatBytesForMessage(MODEL_LOAD_HARD_CAP_BYTES)} load cap."
