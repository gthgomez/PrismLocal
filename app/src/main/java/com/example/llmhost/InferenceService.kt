...
import com.example.llmhost.engine.EngineConfigStore
import com.example.llmhost.chat.ChatManager
import com.example.llmhost.chat.ChatSearchIndex
import com.example.llmhost.chat.TranscriptStore
import com.example.llmhost.export.ChatExporter
import com.example.llmhost.model.DeviceMemorySnapshot
import com.example.llmhost.model.DeviceProfiler
import com.example.llmhost.model.ModelDownloadManager
import com.example.llmhost.model.ModelImportManager
import com.example.llmhost.model.ModelManager
import com.example.llmhost.model.ModelReadinessAssessor
import com.example.llmhost.generation.GenerationMetrics
import com.example.llmhost.generation.GenerationOrchestrator
import com.example.llmhost.generation.PromptBuilder
import com.example.llmhost.agent.AgentTrace
import com.example.llmhost.agent.AgentToolConfirmation
import com.example.llmhost.agent.AgentToolRouter
import com.example.llmhost.util.FormatUtils
...
class InferenceService : Service() {
    ...
    private val configStore = EngineConfigStore(getSharedPreferences(PREFS_NAME, MODE_PRIVATE))
    ...
    private val chatManager = ChatManager(this, transcriptStore, chatSearchIndex, uiState, eventBus)
    ...
    private lateinit var benchmarkStore: com.example.llmhost.benchmark.BenchmarkStore
    private lateinit var benchmarkRunner: com.example.llmhost.benchmark.BenchmarkRunner

    // ── Extracted agent tool handlers (Phase D refactor) ───────────────────
    private lateinit var chatTools: com.example.llmhost.agent.tools.ChatTools
    private lateinit var modelTools: com.example.llmhost.agent.tools.ModelTools
    private lateinit var runtimeTools: com.example.llmhost.agent.tools.RuntimeTools
    private lateinit var memoryTools: com.example.llmhost.agent.tools.MemoryTools
    private lateinit var ragHandlerTools: com.example.llmhost.agent.tools.RagTools
    private lateinit var knowledgePackHandlerTools: com.example.llmhost.agent.tools.KnowledgePackTools
    private lateinit var voiceHandlerTools: com.example.llmhost.agent.tools.VoiceTools
    private lateinit var dataConnectorHandlerTools: com.example.llmhost.agent.tools.DataConnectorHandlerTools
    private lateinit var backgroundAgentHandlerTools: com.example.llmhost.agent.tools.BackgroundAgentTools
    private lateinit var webSearchTools: com.example.llmhost.agent.tools.WebSearchTools
    private lateinit var systemHandlerTools: com.example.llmhost.agent.tools.SystemTools
    ...
    override fun onCreate() {
        ...
        chatTools = com.example.llmhost.agent.tools.ChatTools(
            chatManager = chatManager,
        ...
        modelTools = com.example.llmhost.agent.tools.ModelTools(
            modelManager = modelManager,
        ...
        runtimeTools = com.example.llmhost.agent.tools.RuntimeTools(
            uiState = uiState,
            deviceProfiler = deviceProfiler,
        ...
        memoryTools = com.example.llmhost.agent.tools.MemoryTools(
            memoryStore = memoryStore,
            currentChatId = { _currentChatId.value },
            refreshMemoriesList = { refreshMemoriesList() },
        )
        ragHandlerTools = com.example.llmhost.agent.tools.RagTools(
            ragManager = ragManager,
            vectorStore = vectorStore,
        )
        knowledgePackHandlerTools = com.example.llmhost.agent.tools.KnowledgePackTools(
            knowledgePackManager = knowledgePackManager,
            grokipediaClient = grokipediaClient,
        )
        voiceHandlerTools = com.example.llmhost.agent.tools.VoiceTools(
            voiceIoManager = voiceIoManager,
            uiState = uiState,
        )
        dataConnectorHandlerTools = com.example.llmhost.agent.tools.DataConnectorHandlerTools(
            dataConnectorTools = dataConnectorTools,
        )
        backgroundAgentHandlerTools = com.example.llmhost.agent.tools.BackgroundAgentTools(
            backgroundAgentManager = backgroundAgentManager,
        )
        webSearchTools = com.example.llmhost.agent.tools.WebSearchTools(
            onFirstUse = { publishUiEvent("Web search: DuckDuckGo") },
        )
        systemHandlerTools = com.example.llmhost.agent.tools.SystemTools(
            uiState = uiState,
            eventBus = eventBus,
            deviceProfiler = deviceProfiler,
            modelStorageManager = modelStorageManager,
        ...
        benchmarkStore = com.example.llmhost.benchmark.BenchmarkStore(
            uiState = uiState,
            metrics = generationMetrics,
            deviceProfiler = deviceProfiler,
            filesDir = filesDir,
        ...
        benchmarkRunner = com.example.llmhost.benchmark.BenchmarkRunner(
            uiState = uiState,
            eventBus = eventBus,
            engine = engine,
            metrics = generationMetrics,
    ...
    private fun loadGenerationSettings() {
        _generationSettings.value = configStore.load()
    }
